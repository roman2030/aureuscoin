...
const string strMessageMagic = "AureusCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
    ...
    if (pindexBest->nHeight < 1) //Premine
        ...
        nSubsidy = 0 * COIN; // zero for these blocks to help start
    }   
    ...
    else if(pindexBest->nHeight > 1499999) {
        nSubsidy = 50 * COIN;
    }
...
static const int POW_BLOCK_COUNT = 2000000;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight > 500000) 
			nSubsidy =	nCoinAge * nRewardCoinYear / 365 / COIN / 10;  //end high reward
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AureusCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "AureusCoin New Hash summer 2015";
        CTransaction txNew;
        txNew.nTime = 1437940000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
